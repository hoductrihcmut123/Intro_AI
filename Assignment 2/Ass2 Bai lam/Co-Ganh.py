...
def move(prev_board,board,player,remain_time_x,remain_time_o):
    ...
    if prev_board is None:
        preAction=(None,None)
        root=Node(board,player,preAction,prev_board)         
    else:
        preAction=trackMove(prev_board,board)   
        root=Node(board,player,preAction,prev_board)    
    choose= alpha_beta_prunning(maximizingPlayer,alpha,beta,MAX_DEPTH,root)    
    PREBOARD = board
    BOARD=update(board,choose[1][0],choose[1][1],player)    
...
print(move(PREBOARD,initBoard(), PLAYER, 100, 100))
for i in range(100):
    PLAYER = - PLAYER
    if(PLAYER == -1):
        print("PLAYER 1:\n")
        print(move(PREBOARD, BOARD, PLAYER, 100, 100))
    else:
        print("PLAYER 2:\n")
        print(move(PREBOARD, BOARD, PLAYER, 100, 100))
